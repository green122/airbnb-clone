...
export const getRentals = createSelector(
  getRentalsState,
  (rentalsState: IRentalState) => rentalsState.entities
);
// export const getRentalById = createSelector(getRentalsState, id => (id), (rentalsState, id) => ((rentalsState || {}).detailedRentals || {})[id])
export const getRentalById = createSelector(
  getRentalsState,
  (_: any, id: string) => id,
  (rentalsState, id) => {
...
export const rentalReducer = handleActions<IRentalState, any>(
  {
    ...
    [FETCH_RENTALS_SUCCESS]: (state: IRentalState, action: AnyAction) => ({
      ...state,
      loading: false,
      entities: action.payload
    }),
    [FETCH_RENTAL_SUCCESS]: (state: IRentalState, action: AnyAction) =>
      setRentalById({
        state,
